...
class MCP:
 def __init__(self):
  env=dict(os.environ,XDG_RUNTIME_DIR=str(ROOT/'local/runtime'))
  self.p=subprocess.Popen(['/opt/relay/bin/relay','--experimental','--mode','mcp','--config',str(ROOT/'client.json')],stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=(ROOT/'mcp.log').open('ab'),text=True,env=env)
  self.seq=0
