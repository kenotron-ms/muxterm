...
root.mkdir(mode=0o700)  # Refuse reuse: each verification needs fresh daemons.
client,worker=secrets.token_hex(32),secrets.token_hex(32)
for name,content in {
 'broker':dict(host='sandbox:fixture',clientToken=client,workerToken=worker),
 'client':dict(host='sandbox:fixture',url='https://127.0.0.1',token=client),
 'worker':dict(host='sandbox:fixture',url='https://10.222.0.1',token=worker),
}.items():
...
launch('broker',['/opt/relay/bin/relay','--experimental','--mode','broker','--config',str(root/'broker.json')])
time.sleep(.3)
