...
spec=importlib.util.spec_from_file_location('smoke','/home/ken/artifacts/live-mcp.py');smoke=importlib.util.module_from_spec(spec);spec.loader.exec_module(smoke)
MCP=smoke.MCP
cfg=json.loads((root/'client.json').read_text())
def call_http(path,body=None,token=None):
 conn=http.client.HTTPSConnection('10.9.113.136',context=ssl.create_default_context(cafile='/home/ken/artifacts/sandbox-live-ca.pem'),timeout=25)
...
def fault(mode,marker=''):
 c=http.client.HTTPConnection('127.0.0.1',33985,timeout=5);c.request('POST','/__fault',json.dumps(dict(mode=mode,marker=marker)));res=c.getresponse();res.read();c.close()
def state():
 c=http.client.HTTPConnection('127.0.0.1',33985,timeout=5);c.request('GET','/__fault');r=json.loads(c.getresponse().read());c.close();return r
suffix=str(time.time_ns())
uncertain_path='/tmp/relay-uncertain-'+suffix
machine='sandbox:live-local'
...
for _ in range(50):
 observed=subprocess.run(['incus','exec','muxterm-sandbox-live-worker','--','cat',uncertain_path],capture_output=True,text=True)
 if observed.returncode==0:break
 time.sleep(.05)
assert observed.stdout=='UNCERTAIN_INPUT\n'
subprocess.run(['incus','exec','muxterm-sandbox-live-worker','--','python3','-c',"import json,pathlib,os,signal; p=json.loads(pathlib.Path('/opt/relay/pids.json').read_text())['agent']; assert b'/opt/relay/bin/agent' in pathlib.Path(f'/proc/{p}/cmdline').read_bytes(); os.kill(p,signal.SIGKILL)"],check=True)
fault('')
...
assert call_http('/discover',token=worker['enrollmentToken'])[0]==401
...
m.close()
print('Secure broker mode used real Entra validation and fresh single-use enrollment after worker crash.')
(root/'fault-results.json').write_text(json.dumps(dict(worker_dedup=True,lost_ack=True,sse_reconnect=True,uncertain_crash_no_replay=True,authorization=True,ordered_input=True,broker_restart=False)))
