...
The canonical top-right title action order is **[voice circle + FOUR rounded vertical bars] [...]**. In the wide workspace view, this pair is a fixed trailing action group in the topmost, rightmost Dockview tab strip, outside scrolling tabs; split groups do not duplicate it. Mission Control retains its top-right pair. The existing narrow/mobile shared title and pane picker preserve their accepted spacing. Empty workspaces use the existing title fallback. The button remains visible when unavailable, with an explanatory accessible description; `Start voice mode` is the only activation intent. Connecting is pending, not listening. While active the title control is pressed and labelled `Stop voice mode`, even when composer state or navigation is busy. No composer-local voice MODE activators remain; composer dictation retains its separate mic.
...
The floating bubble's native circular button owns the single outer border. Its interior omits the nested SVG circle; the normal header icon keeps that circle. Active unmuted Listening/Thinking/Speaking uses the green outer border; Connecting, Mic muted and Error remain visually distinct. This is a presentation refinement only, not another activation control or voice lifetime.
...
## Workspace screen tooltip — 2026-09-13 refinement

Workspace sidebar rows retain their name, status and close controls, without permanent terminal thumbnails. One delayed hover or keyboard-focus tooltip shows the entire current screen grid of the workspace's last user-selected pane, using the existing tiny pixel font and terminal color/layout renderer. It is not scrollback, a line-count sample, a cursor crop, a combined split-pane image or the most recently output-producing pane. The grid's pixel-cell geometry determines its aspect ratio and natural size; uniform viewport fitting preserves every row and column, including the bottom-right cell.

The snapshot request is bounded, read-only and on demand. It neither attaches to a workspace nor changes focus, PTY dimensions or input. Selected-pane identity is separate from sizing-authority claims for all visible split panes. Unknown source, unsupported daemon or oversized screen yields explicit unavailability rather than another pane or a cropped substitute. After a daemon restart, a normal selected-pane focus establishes that identity again.

Leave, blur, Escape, navigation and disconnect cancel or dismiss the preview; stale replies cannot replace a newer target. Hover causes no sidebar reflow or continuous hidden-workspace polling. Ordinary touch taps still navigate and do not initiate previews. Fleet/task cards and running workspaces, panes, preview servers and tunnels are not removed by this presentation change.
