...
- **Pre-existing boundary quirk (out of scope for this design):** `breakpoint.ts`'s `layoutModeForWidth` treats `width >= 768` as wide, but `mux-dock.ts`'s media query hiding the dockview header is `@media (max-width: 768px)` (inclusive of 768 itself). At a viewport width of exactly 768px, the JS layer renders wide mode (sidebar + non-narrow dockview) while the CSS layer still force-hides the dockview header — a genuine boundary contradiction that already exists in the current codebase, unrelated to touch-target sizing and not introduced by this design. Fixing it here would be unrequested scope creep per YAGNI, so it is documented but left untouched.
...
### Issue 2b: New-workspace menu entry (kebab, narrow-mode only)

- `<mux-launcher-menu>` is a **shared** component: it's instantiated both by `<mux-title-bar>` (narrow mode) and by `<mux-sidebar>` (wide mode, its own `.launcher-btn` + `@launcher-action` listener at `mux-sidebar.ts:507-515`). Adding "New workspace" unconditionally would leak it into the wide-mode desktop kebab too, contradicting Issue 1's "wide/desktop mode: zero changes" scope boundary and duplicating `mux-sidebar`'s existing always-visible "+ New workspace" button. To avoid this, rendering is gated by a new boolean property.
- `launcher-menu.ts`: extend the `LauncherAction` type to add `'new-workspace'` (only this one new value — new-pane does not go through this menu, see above). Add a new boolean property `showCreateWorkspace` (default `false`). When `showCreateWorkspace` is `true`, render one new `<button data-action="new-workspace">` dispatching `this._dispatch('new-workspace')`; when `false` (the default), the menu renders exactly its original 4 items. Placement when shown: at the top of the menu list, followed by a `<div class="divider">` before the existing Settings/Shortcuts/Reconnect items — mirroring `mux-sidebar.ts`'s convention of visually separating "+ New workspace" (content-creation) from utility actions. Icon: `Plus` from lucide (same icon as the title-bar new-pane button, for visual consistency that "+" always means "create").
- `title-bar.ts`: the only call site that passes `.showCreateWorkspace="${true}"` when instantiating `<mux-launcher-menu>` — this is the narrow-mode context where the action is needed.
- `mux-sidebar.ts`: its existing instantiation of `<mux-launcher-menu>` is left unchanged — it passes nothing, so `showCreateWorkspace` defaults to `false` and its kebab menu is exactly the original 4 items.
...
- **Wide-mode kebab stays untouched:** `<mux-launcher-menu>`'s new `showCreateWorkspace` property (see Issue 2b) defaults to `false` and is only ever set `true` by `<mux-title-bar>`. This is a regression-prevention property of the design, not just an implementation detail — it's the mechanism that guarantees `<mux-sidebar>`'s wide-mode kebab menu renders exactly as it did before this change, with no unconditional leak of the new menu item into desktop.
- **Menu closes before modal opens:** tapping "New workspace" in the kebab closes the menu before the create modal opens. This is already the existing behavior of `title-bar.ts`'s `_onLauncherAction` handler (`this._menuOpen = false` runs unconditionally before re-dispatching, for all actions) — a documentation clarification only, not new code.
...
**Desktop viewport pass (wide, 1280x800) — regression check:**

Uses an explicit 1280x800 viewport, well clear of the breakpoint, rather than an ambiguous "≥768px" framing — this avoids confounding the regression results with the pre-existing exact-768px boundary quirk documented in Current State (at exactly 768px, `breakpoint.ts` reports wide mode while `mux-dock.ts`'s CSS still force-hides the dockview header, since `max-width: 768px` is inclusive).

5. Fresh workspace/pane again (hygiene rule applies to every run, not just mobile).
6. **(d)** Confirm `<mux-sidebar>` renders (not title-bar), dockview tab strip height is completely unchanged from before this change, sidebar "+ New workspace" still works, dockview's own "+" still works, and the wide-mode kebab menu (`<mux-launcher-menu>` instantiated by `<mux-sidebar>`) shows only its original 4 items with no "New workspace" entry.
