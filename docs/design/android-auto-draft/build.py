...
<b>the same row titles</b>. Only the secondary text and toggle state may change for free.</p>
...
<tr><td><b>5. What a tap on a row does</b></td>
...
<b>Speaks the row aloud</b>. The design guidance says information-only rows are discouraged, which argues
...
def main():
    doc = HEAD + TRUTH + build() + OVERFLOW + DAYNIGHT + DECIDE
