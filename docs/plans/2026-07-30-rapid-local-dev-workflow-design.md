...
| sessiond socket dir | `$TMPDIR/muxterm-501/` (default, no `XDG_RUNTIME_DIR`) | `<worktree>/tmp/muxterm-dev-runtime/muxterm/` (`XDG_RUNTIME_DIR` set) |
...
3. **Different sessiond socket dir** -- `XDG_RUNTIME_DIR` redirects the dev instance's `sessiond.sock`, `sessiond.log`, and `server.url` into a worktree-local directory that production's `socketDir()` (no env var set) will never resolve to.
...
# air, so the sessiond this spawns uses tmp/muxterm-dev-runtime/ instead of
# the default $TMPDIR/muxterm-<uid>/ where the native companion app's
# production sessiond lives. Never run this without that env var set.
#
...
2. Makefile creates `tmp/muxterm-dev-runtime/`, exports `XDG_RUNTIME_DIR`, backgrounds `vite build --watch` (writes to `web/dist/`, touches `build.stamp` on full flush), starts `air -c .air.local.toml` in foreground.
...
4. On serve startup, `EnsureDaemon()` dials `tmp/muxterm-dev-runtime/muxterm/sessiond.sock`, finds nothing, spawns a detached sessiond logging to `tmp/muxterm-dev-runtime/muxterm/sessiond.log`, waits until it's reachable.
...
7. Developer hits Ctrl-C on the foreground air process -> trap kills the backgrounded Vite watcher -> air's own signal handling tears down the `bin/muxterm-dev` child it was supervising. The dev sessiond (spawned detached via `Setsid`, per `spawn.go`'s `SpawnCommand`) may persist after Ctrl-C -- this is expected/harmless, worktree-scoped, and can be cleaned up by deleting `tmp/muxterm-dev-runtime/` if ever desired.
...
- **Stale dev sessiond from a previous session:** handled generically by the existing, unmodified `EnsureDaemon()` (dials the socket, removes stale socket file if dead, spawns fresh) -- operates only within `tmp/muxterm-dev-runtime/`, never near production's socket dir.
...
3. **Observe startup** -- confirm the printed banner shows `127.0.0.1:8313`, confirm `tmp/muxterm-dev-runtime/muxterm/sessiond.sock` gets created (not `$TMPDIR/muxterm-501/*`), confirm a new PID appears for `bin/muxterm-dev` distinct from 58493/58494.
