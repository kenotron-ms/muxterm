...
Also explicitly considered and rejected: **Dynamic Client Registration (RFC 7591)**. The MCP Authorization spec (2025-06-18) itself anticipates authorization servers that don't support DCR: its documented fallback is a hardcoded, well-known `client_id` that any MCP client's user configures manually in that client's own settings, rather than a runtime registration flow. muxterm has exactly one principal (the OS account) and no per-client scopes or consent differentiation — full access is the only access level, so there is nothing for DCR to differentiate between clients on. Implementing DCR would add a real implementation surface (a `/register` endpoint per RFC 7591, plus client-metadata storage and lifecycle) with no corresponding benefit here, so it is rejected in favor of the two hardcoded clients described below.
...
- **Explicit hardening configuration (not library defaults):** `go-oauth2/oauth2` ships with permissive defaults that must be explicitly overridden, not relied upon:
  - PKCE `S256` is configured as the only permitted code-challenge method — `plain` and no-PKCE authorization requests are rejected.
  - Only the authorization-code grant type is enabled; other grant types the library supports by default (e.g. client-credentials, implicit, resource-owner password) are disabled.
  - Refresh-token issuance is disabled entirely, consistent with the 30-day-token/no-refresh-token model below.
  - The access-token TTL is explicitly set to 30 days rather than relying on the library's (shorter) default.
- **Client model — redirect URI validation shape, not permission level:** the two hardcoded clients are not differentiated by access level or by "which third-party tool" — full access is the only access level, so there is nothing to differentiate on scope. They are differentiated purely by redirect URI validation shape. **The variable-port exception below is bounded to exactly one client (`muxterm-mcp`) and one host (`127.0.0.1`); it is not a general wildcard-matching capability of the redirect URI validator.**
  - `muxterm-web`: exact-match HTTPS redirect URI — muxterm's own `/auth/callback` route (derived from `public_origin`, see "TLS termination" below), first-party, browser-based. This match is fully exact end-to-end — scheme, host, port, and path all fixed — with **no variable component of any kind**.
  - `muxterm-mcp`: the standard OAuth 2.1 native-app/CLI redirect URI pattern — `http://127.0.0.1:<any-port>/<exact-path>`. This is the one, bounded exception to exact redirect URI matching in this design: scheme (`http`), host (`127.0.0.1`), and path are still matched exactly; **only the port is unconstrained**, since each client instance binds its own ephemeral port for a temporary local callback listener held only for the duration of the login flow. No other client, and no other part of the host/scheme/path, is ever wildcarded. This single `client_id` is shared by any MCP client or future native companion app — it is not per-tool and requires no pre-coordination with muxterm; the user just configures the published `muxterm-mcp` client_id in their tool of choice (see "Alternatives Considered" for why Dynamic Client Registration was rejected in favor of this fixed pair).
- Provides a small local file-backed store (alongside `internal/config`'s existing conventions) implementing the library's `TokenStore` interface, for both authorization codes and issued access tokens.
  - **Atomic authorization-code consumption:** consuming an authorization code at `/token` is an atomic check-and-delete operation, not two separate read-then-delete store calls — this prevents a race where concurrent token-exchange requests could both succeed against the same code.
  - **File permissions:** the store's backing files are restricted to owner-only read/write, matching the posture already used for the Unix-socket directory permissions in `internal/sessiond`.
...
- **Token/cookie lifetime:** access tokens and the browser session cookie are **long-lived (e.g. ~30 days)**. There is deliberately **no refresh-token mechanism** — this matches a single-account personal tool where token-theft blast radius is lower stakes than a multi-tenant system, and periodic re-login (via PAM/OpenDirectory/`LogonUser`) is cheap and low-friction. When a token expires, the user simply logs in again through the normal `/authorize` flow. **Acknowledged spec deviation:** the MCP Authorization spec's security considerations state that authorization servers SHOULD issue short-lived access tokens to limit the blast radius of a leaked token. This design deliberately deviates from that SHOULD-level recommendation — muxterm is a single-account personal tool where the periodic-re-login cost of short-lived tokens outweighs the marginal security benefit for this threat model. This is stated here as a conscious, documented trade-off, not an oversight.
- **OAuth 2.0 Authorization Server Metadata (RFC 8414):** `internal/authserver` MUST serve a `.well-known/oauth-authorization-server` document for AS discovery. (This was implicitly assumed in an earlier draft of this design; it is now an explicit stated requirement, per the MCP Authorization spec's discovery expectations.)
...
- **MCP spec-compliance requirements (spec MUSTs, not optional):** `POST /mcp` acts as an OAuth 2.1 Resource Server per the MCP Authorization spec (2025-06-18), which imposes the following requirements beyond bearer-token validation:
  - **OAuth 2.0 Protected Resource Metadata (RFC 9728):** MUST serve a `.well-known/oauth-protected-resource` document indicating the location of muxterm's own authorization server.
  - **`WWW-Authenticate` on 401:** 401 responses from `/mcp` MUST include a `WWW-Authenticate` header pointing to that protected-resource metadata URL, per RFC 9728 §5.1 — this is how compliant MCP clients discover which AS to authenticate against.
  - **Resource Indicators (RFC 8707):** MCP clients MUST send a `resource` parameter — the canonical URI of muxterm's `/mcp` endpoint — in both the authorization request and the token request. muxterm's AS/resource-server MUST validate that a presented access token was actually issued with that resource as its intended audience, and MUST reject tokens issued for any other audience. (No token passthrough to any downstream service applies here, since muxterm doesn't proxy tokens elsewhere — stated for completeness/spec alignment.)
  - **Bearer token transport:** access tokens MUST be sent only via the `Authorization: Bearer <token>` header, never in a URL query string. This already matches the design's stated approach in Architecture/Components; this bullet makes it an explicit stated MUST, referencing the spec.
...
**Invariant applied in both deployment modes: muxterm's own HTTP listener binds `127.0.0.1`-only, full stop.** This is not a direct-mode-specific rule — it is one bind-address invariant that holds identically regardless of deployment mode. In Caddy-fronted mode, Caddy reaches muxterm over loopback exactly like any other local process talking to a local service; nothing remote ever reaches muxterm's raw HTTP port directly, in either mode. The two modes differ only in what sits in front of that always-loopback-bound listener, never in the listener's own bind address.

Two deployment modes:

1. **Direct/local-dev mode:** muxterm listens on plain HTTP directly, with no proxy in front, **bound to `127.0.0.1` only** — this is a change from muxterm's current default bind address of `0.0.0.0`. The existing loopback bypass (`RemoteAddr`-based, via `IsLocalhost()`-equivalent logic in the new middleware) applies normally to genuinely local same-box traffic. **Public/remote reachability requires the Caddy-fronted HTTPS mode below.** Direct mode is genuinely local-only by bind address, not merely by an auth-logic bypass: a socket bound to `127.0.0.1` cannot accept a remote connection at the OS/network level at all, regardless of what the auth middleware would otherwise decide.
2. **Caddy-fronted HTTPS mode:** Caddy terminates TLS and reverse-proxies to muxterm's HTTP listener — which, per the invariant above, remains bound to `127.0.0.1` exactly as in direct mode; only Caddy itself binds a public-facing address. In this mode, **muxterm's loopback bypass MUST be explicitly disabled** via an opt-in config flag (e.g. `behind_reverse_proxy: true` — exact name is an implementation detail). This is **not** auto-detected and **not** based on trusting a forwarded-for header (headers are spoofable, and this design already rejected trusting them elsewhere). When this flag is set, all traffic reaching muxterm authenticates through the AS regardless of apparent origin — because Caddy's own loopback connection to muxterm would otherwise look identical to genuinely local traffic and silently grant the bypass to what might be a genuinely remote, unauthenticated caller, defeating the entire point of this design.

   In this mode, the operator MUST also set **`public_origin`** (e.g. `public_origin: "https://muxterm.example.com"`) — the canonical public HTTPS origin at which muxterm is reachable through Caddy. muxterm cannot infer this value itself: its own listener only ever sees Caddy's local loopback connection, never the public-facing request, and this design already rejects deriving trust-relevant values from client-supplied/forwarded headers (they are spoofable). `public_origin` is therefore a required, explicitly-configured value in this mode, and it is the single source from which every other public-facing URL in this design is derived:
   - `muxterm-web`'s exact-match redirect URI (`<public_origin>/auth/callback` — see "Client model" above).
   - The RFC 8414 `.well-known/oauth-authorization-server` metadata document's issuer/endpoint URLs (see `internal/authserver` above).
   - The RFC 9728 `.well-known/oauth-protected-resource` metadata document's resource identifier (see `POST /mcp` above).
   - The canonical `resource` URI for `/mcp` used in the RFC 8707 resource-indicator flow (see `POST /mcp` above and Data Flow below).

   **In direct/local-dev mode, `public_origin` is not needed and not applicable** — there is no external origin in that mode; the equivalent URLs are constructed from the loopback address instead (e.g. `http://127.0.0.1:<port>`).

This is a **security-critical configuration requirement**, not an optional nicety: deploying muxterm behind Caddy (or any reverse proxy) without setting `behind_reverse_proxy` and `public_origin` reopens the exact vulnerability class this whole design exists to close, or produces incorrect/unusable metadata and callback URLs.
...
**MCP-over-HTTP:**
1. The MCP client briefly binds a local `http://127.0.0.1:<ephemeral-port>/callback` listener, then opens the system browser to `/authorize?client_id=muxterm-mcp&response_type=code&code_challenge=...&redirect_uri=http://127.0.0.1:<ephemeral-port>/callback&resource=<canonical /mcp URI>` — PKCE, native-app redirect pattern, matching the pattern used by tools like `gh auth login` (not the earlier, vaguer notion of the token simply being "held by the MCP client").
2. `authserver` renders the password form; on success, the browser is redirected back to the MCP client's temporary local listener with `code` + `state`.
3. The MCP client exchanges `code` + the PKCE verifier at `/token` — including the same `resource` parameter — receiving an access token, then shuts down its temporary listener.
4. The resulting access token is held by the MCP client itself (its own config/credential storage) and sent as `Authorization: Bearer <token>` on every `POST /mcp` call — no cookie involved.
5. Client id `muxterm-mcp` is a single pre-registered, published value shared by any MCP client or native companion app; no dynamic client registration is supported (see "Alternatives Considered" for why).
...
- **Invalid/expired access token on a protected route — 401 handling mechanics:** the server itself never issues an HTTP redirect for this case; all callers get a plain `401 {"error":"invalid_token"}` JSON body. The redirect-to-login behavior is a client-side concern, split by call shape:
  - For `fetch`/API/bearer callers: the SPA's own client-side JS is responsible for detecting a 401 from any of its API calls and redirecting `window.location` to `/authorize` itself. The server's job ends at returning the 401 body.
  - For the `/ws` upgrade specifically: a WebSocket upgrade response can't cleanly carry a JSON body the way a `fetch` response can, and a 401 there would be indistinguishable from an ordinary dropped/failed WebSocket connection (which the client already has separate reconnect-retry logic for). To avoid that ambiguity, the SPA performs an auth-check API call (e.g. hitting a lightweight authenticated endpoint) *before* attempting to open the WebSocket connection at all — an expired session is caught and redirected there, never inside the WebSocket handshake itself.
  Since tokens and the session cookie are long-lived (~30 days, no refresh-token mechanism — see `internal/authserver` in Architecture/Components), this expiry path is only reached roughly monthly in normal use; the user's only recourse on expiry is a normal re-login through `/authorize`, which is treated as cheap and expected, not an error condition to work around.
- **Redirect URI validation:** Strictly matched against the pre-registered URI for each of the two hardcoded clients. `muxterm-web`'s redirect URI is a fully exact match with no variable component at all. The sole bounded exception is `muxterm-mcp`: scheme (`http`), host (`127.0.0.1`), and path are still matched exactly, and only the port is unconstrained (`http://127.0.0.1:<any-port>/<exact-path>`) — see "Client model" in Architecture/Components. No other wildcarding exists anywhere in redirect URI validation. This prevents open-redirect attacks via a crafted `redirect_uri` parameter.
...
- Dynamic OAuth client registration — exactly two hardcoded clients (`muxterm-web`, `muxterm-mcp`) for the foreseeable future (see "Alternatives Considered" for why DCR was evaluated and rejected).
- **`/t/*` tunnel credential exposure — only partially mitigated, not fully solved.** The header-stripping fix in Auth middleware (component 3) closes the credential-*forwarding* vector: the tunnel proxy no longer forwards `Cookie`/`Authorization` to the tunneled target. It does **not** close a deeper vector: content rendered under `/t/{id}/...` shares muxterm's own origin, so that tunneled page's own JavaScript can make same-origin `fetch`/WebSocket calls directly to `/api/*` or `/ws` — the browser will auto-attach muxterm's session cookie to those requests regardless of the tunnel proxy's own header-stripping, since that traffic never goes through the tunnel proxy hop at all. A full fix requires serving tunneled content on a genuinely separate origin (a different port or subdomain per tunnel) so browser same-origin protections actually apply — this is a real architectural change to the tunnel feature and is explicitly out of scope for this authentication design. This is stated here honestly as an unresolved, pre-existing risk (it predates this design) that becomes more consequential now that a real, long-lived session credential — not a 30-second throwaway HMAC token — sits behind the cookie. This design does not claim to have fully solved it.
