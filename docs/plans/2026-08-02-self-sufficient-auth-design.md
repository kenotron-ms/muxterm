...
- **Client model — redirect URI validation shape, not permission level:** the two hardcoded clients are not differentiated by access level or by "which third-party tool" — full access is the only access level, so there is nothing to differentiate on scope. They are differentiated purely by redirect URI validation shape:
  - `muxterm-web`: exact-match HTTPS redirect URI — muxterm's own `/auth/callback` route, first-party, browser-based.
  - `muxterm-mcp`: the standard OAuth 2.1 native-app/CLI redirect URI pattern — `http://127.0.0.1:<any-port>/...`, with the port left wildcarded/unvalidated (only scheme=`http`, host=`127.0.0.1`, and path are validated exactly), since each client instance binds its own ephemeral port for a temporary local callback listener held only for the duration of the login flow. This single `client_id` is shared by any MCP client or future native companion app — it is not per-tool and requires no pre-coordination with muxterm; the user just configures the published `muxterm-mcp` client_id in their tool of choice (see "Alternatives Considered" for why Dynamic Client Registration was rejected in favor of this fixed pair).
