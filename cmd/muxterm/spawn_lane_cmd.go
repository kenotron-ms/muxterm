...
func runSpawnLane(args []string) error {
	...
	fs.Usage = func() {
		...
		fmt.Fprintln(os.Stdout, "")
		fmt.Fprintln(os.Stdout, "--goal runs 'amplifier run \"/goal <condition>\"' \u2014 headless, with NO --mode")
		fmt.Fprintln(os.Stdout, "chat, because /goal is only honoured on amplifier's headless path. The lane")
		fmt.Fprintln(os.Stdout, "carries its own declared stop condition (visible as doneMeans in 'muxterm")
		fmt.Fprintln(os.Stdout, "fleet') and loops until the condition is met.")
		fmt.Fprintln(os.Stdout, "")
		fmt.Fprintln(os.Stdout, "When the loop ends the pane does NOT close: it resumes that same session")
		fmt.Fprintln(os.Stdout, "interactively, holding the whole run's context, so you can read the verdict")
		fmt.Fprintln(os.Stdout, "and keep working from where it stopped. A lane that got most of the way")
		fmt.Fprintln(os.Stdout, "there is a lane you can finish by typing at it. Ctrl-D ends the lane and")
		fmt.Fprintln(os.Stdout, "closes the pane; until then the workspace stays, so goal lanes accumulate")
		fmt.Fprintln(os.Stdout, "workspaces and are yours to close.")
		fmt.Fprintln(os.Stdout, "")
		fmt.Fprintln(os.Stdout, "An interactive lane (no --goal) keeps --mode chat and stays alive. Claude")
		fmt.Fprintln(os.Stdout, "Code has no goal mode and rejects --goal rather than ignoring it.")
		fmt.Fprintln(os.Stdout, "")
