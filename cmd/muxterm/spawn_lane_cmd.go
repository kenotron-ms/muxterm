...
func runSpawnLane(args []string) error {
	...
	fs.Usage = func() {
		...
		fmt.Fprintln(os.Stdout, "")
		fmt.Fprintln(os.Stdout, "--goal launches 'amplifier run \"/goal <condition>\"' \u2014 headless, with NO")
		fmt.Fprintln(os.Stdout, "--mode chat, because /goal is only honoured on amplifier's headless path.")
		fmt.Fprintln(os.Stdout, "The lane then carries its own declared stop condition (visible as doneMeans")
		fmt.Fprintln(os.Stdout, "in 'muxterm fleet'), loops until the condition is met, and exits. Its pane")
		fmt.Fprintln(os.Stdout, "closes on that exit and the fleet row goes with it \u2014 watch a goal lane while")
		fmt.Fprintln(os.Stdout, "it runs, not after. An interactive lane (no --goal) keeps --mode chat and")
		fmt.Fprintln(os.Stdout, "stays alive. Claude Code has no goal mode and rejects --goal rather than")
		fmt.Fprintln(os.Stdout, "ignoring it.")
		fmt.Fprintln(os.Stdout, "")
