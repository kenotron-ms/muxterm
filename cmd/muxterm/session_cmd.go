...
func runSession(args []string) error {
	if len(args) == 0 || args[0] == "--help" || args[0] == "-h" {
		...
		fmt.Fprintln(os.Stdout, "  list                    Legacy alias for 'muxterm workspace list' (terminal workspaces)")
		fmt.Fprintln(os.Stdout, "  attach <workspace-id>   Inspect a terminal workspace's panes and layout")
	...
	case "hook-report":
		return runSessionHookReport(args[1:])
	case "codex-notify":
...
func runSessionAttach(args []string) error {
	...
	fs.Usage = func() {
		...
		fmt.Fprintln(os.Stdout, "")
		fmt.Fprintln(os.Stdout, "Inspect a terminal workspace's composition (panes and saved layout).")
		fmt.Fprintln(os.Stdout, "")
