...
// coding-agent CLI" actually true rather than aspirational. Managed Amplifier,
// Claude and Codex launches use native hooks, but those do not help a Makefile,
// a nightly shell script, a CI step, or somebody's Rust binary. This verb is the
// answer for all of them: one command, no library, no language binding, no
// daemon connection. It writes one snapshot to the spool and exits.
//
